		...
		if(edtBarcodeValue->Text.Length() == 12)
		{
			if(StrProc->CheckStringIsMAC(edtBarcodeValue->Text))
			{
				...
				|| plResult->Color != clCream)
				{
					...
					{
						case KEYIN_PASS:
							lbErrorMsg->Caption = "Barcode scanned successfully";
							lbErrorMsg->Font->Color = clBlue;
							writeLOG("PASS");
							break;
						case KEYIN_REPEAT_MAC:
							lbErrorMsg->Caption = "(!)MAC碼已重複";
							lbErrorMsg->Font->Color = clRed;
					...
					if(lbErrorMsg->Font->Color == clRed)
					{
...
void TfrmMain::ShowWorkOrderForm()
{
	...
	if(frmMsg->ShowModal()== mrOk)
	{
		...
		//
		LOGFilePath = "C:\\ASMP\\log\\"+plWorkOrderNumber->Caption+"\\MAC-Mgmt(OnlyMAC)";
		...
		if (lanfile.is_open()) {
			while (!lanfile.eof())
			{
				...
				if (strstr(filemsg.c_str(), "PASS"))
				{
					...
					LOG_MAC = LOG_MAC.SubString(LOG_MAC.Pos("[MAC]")+6,LOG_MAC.Pos(",[StartCode]")-LOG_MAC.Pos("[MAC]")-6).Trim();
					if(plBurnMAC->Caption == LOG_MAC && !bInit)
					{
						lanfile.close();
...
void __fastcall TfrmMain::edtBarcodeValueClick(TObject *Sender)
{
	edtBarcodeValue->SelectAll();
}
//---------------------------------------------------------------------------
